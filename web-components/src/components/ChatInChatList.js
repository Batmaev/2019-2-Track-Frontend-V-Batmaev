...
class ChatInChatList extends HTMLElement {
    ...
    connectedCallback() {
        ...
        const lastMessage = this.message_list.firstChild
        this.text.innerText = lastMessage.getAttribute("text")
        this.time.innerText = lastMessage.getAttribute("time")

        const statusFromAttribute = this.getAttribute("status")
        switch (statusFromAttribute) {
            case "sent_and_read":
    ...
    ShowMessages() {
        ...
        this.left_button.addEventListener('click', this.ShowDialogs.bind(this))
